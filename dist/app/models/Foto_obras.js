"use strict";Object.defineProperty(exports, "__esModule", {value: true}); function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }var _sequelize = require('sequelize'); var _sequelize2 = _interopRequireDefault(_sequelize);

class Foto_obras extends _sequelize.Model {
  static init(sequelize) {
    super.init(
      {
        id_foto_obras: {
          type: _sequelize2.default.INTEGER,
          primaryKey: true,
          autoIncrement: true,
        },
        descricao: _sequelize2.default.TEXT,
        data: {
          type: _sequelize2.default.DATE,
          defaultValue: _sequelize2.default.literal(
            "CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3)"
          ),
        },
        foto: _sequelize2.default.STRING(50),
        titulo: _sequelize2.default.STRING(30),
        fk_artista_id_artista: _sequelize2.default.INTEGER,
      },
      {
        timestamps: false,
        sequelize,
      }
    );
    return this;
  }

  static associate(models) {
    this.belongsTo(models.Artista, { foreignKey: "fk_artista_id_artista" });
  }
}

exports. default = Foto_obras;
